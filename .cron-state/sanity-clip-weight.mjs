...
eq(clipWeight({ kind: "text", content: "hello" }), { chars: 5, bytes: 5 }, "ascii chars=bytes");
eq(clipWeightSummary({ kind: "text", content: "hello" }), "5 chars \u00b7 5 B", "ascii summary");

// --- singular grammar ---
eq(clipWeightSummary({ kind: "text", content: "x" }), "1 char \u00b7 1 B", "1 char singular");
...
eq(clipWeightSummary({ kind: "text", content: "a😀b" }), "3 chars \u00b7 6 B", "emoji summary bytes>chars");

// --- CJK: 1 code point = 3 UTF-8 bytes ---
eq(clipWeight({ kind: "text", content: "中文" }), { chars: 2, bytes: 6 }, "CJK 2 chars 6 bytes");

// --- KB tier formatting matches formatCopyBytes (1500 B -> 1.5 KB) ---
eq(clipWeightSummary({ kind: "text", content: "x".repeat(1500) }), "1,500 chars \u00b7 1.5 KB", "KB tier + grouping");

// --- grouping at thousands ---
eq(clipWeightSummary({ kind: "text", content: "x".repeat(1240) }), "1,240 chars \u00b7 1.2 KB", "1240 grouped");

// --- link clips are eligible (they have a text body) ---
eq(clipWeightSummary({ kind: "link", content: "https://example.com" }), "19 chars \u00b7 19 B", "link eligible");
...
eq(w.payload, "11 chars \u00b7 11 B", "weight payload is WYSIWYG summary");
